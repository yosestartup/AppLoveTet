...
struct Environment {
    static var baseUrl: String = Environment.environmentVariable(environment: .development) ?? ""
  
  static func environmentVariable(environment: EnvironmentType) -> String? {
      let processInfo = ProcessInfo.processInfo
    guard let value = processInfo.environment[environment.rawValue] else {
          return nil
      }
      return value
  }
